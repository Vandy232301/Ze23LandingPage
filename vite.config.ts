...
export default defineConfig({
  plugins: [
    ...
    tailwindcss(),
    {
      name: 'figma-asset-transformer',
      enforce: 'pre',
      transform(code, id) {
        if (!id.endsWith('.tsx') && !id.endsWith('.ts') && !id.endsWith('.js')) return;
        ...
        for (const [hash, url] of Object.entries(assetMap)) {
          ...
          const regex = new RegExp(`figma:asset/${escapedHash}(\\.[a-z0-9]+)?`, 'g');
          newCode = newCode.replace(regex, url);
        }
        
        // Handle any remaining figma:assets with a transparent pixel to prevent build failure
        newCode = newCode.replace(/figma:asset\/[a-zA-Z0-9]+(\.[a-z0-9]+)?/g, 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7');
